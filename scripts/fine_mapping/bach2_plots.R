...
peak1 = list(label="chr6_90266766_90267715", chr="chr6", start=90266766, end=90267715)
...
# peaks_all[peaks_all$chromosome=="chr6" & peaks_all$start<90296024 & peaks_all$end>90296024,]
...
dp3_counts = readRDS(paste(dp3_dir,"processed_countMat_clean_voomnormalized.rds",sep="/"))
dp3_geno = readRDS(paste(dp3_dir,"mega_genotypes/mega_genotype_matrix.rds",sep="/"))
dp3_sampleinfo = readRDS(paste(dp3_dir,"processed_sampleDF.rds",sep="/"))
...
getAlleleImbalance = function(variant) {
  vardir = paste0(Sys.getenv("qtl"),"/allelic_bias/",variant[["rsid"]])
...
getLCPlot = function(feature, label) {
  gwas_fn = paste0(Sys.getenv("qtl"),"/locuscompare/lcformat_",feature,"_gwas.txt")
  caqtl_fn = paste0(Sys.getenv("qtl"),"/locuscompare/lcformat_",feature,"_qtl.txt")
  locuscompare(in_fn1 = gwas_fn, in_fn2 = caqtl_fn, title = 'GWAS', title2 = label, combine=FALSE)
}
pdf("bach2_coloc.pdf")
getLCPlot(peak1[["label"]], "caQTL")
getLCPlot("BACH2", "BACH2 eQTL")
dev.off()
...
p_main1 = plot_grid(p_credset,p_gene,p_hic, getChromHMMPlot2(blu_reg, region_broad, blukey_tissue, blukey_state, snp_lines),blankPlot, nrow=5, ncol=1, align="v", axis="lr", rel_heights=c(1,1,1,4,0.5))
