...
#including the tag variants in the haplotype plots whenever the index variant has PP<0.5:
...
load(file="/scratch/ji3x/final_collection_gwas/resultsmeta_ind_fam_tdt_5pc_diff.RData")
res$snp<-gsub(":",".",res$MarkerName)
...
s<-summx[summx$MarkerName==index,]
proceed<-ifelse(nrow(s)==0,T,F)
if (nrow(s)>0){
proceed<-ifelse(s$ppsum<0.5,T,proceed)
}
...
if(nrow(s)==0){
tags<-c(tags,gsub(":",".",index))
}
la<-la[tags,]
cols<-cols[c(1:nrow(la))]
names(cols)<-la$ID
if(nrow(s)==0){
names(cols)[length(cols)]<-orig[orig$Marker==index,"ID"]
...
tg<-creds[creds$MarkerName==index,"tag"][1]
creds<-creds[creds$ppsum>0.5 | creds$tag %in% tg,]
...
snps<-creds$ID
if(nrow(s)==0){
snps<-c(creds$ID,orig[orig$Marker==index,"ID"])
}
cols<-cols[names(cols) %in% snps]
...
if(nrow(s)==0){
tags<-c(unique(creds$tag),gsub(":",".",orig[orig$Marker==index,"Marker"]))
}
if(length(tags)==1){
out<-NULL
}
...
l<-res[res$snp %in% tags,]
rownames(l)<-l$MarkerName

o<-l[,c("chromosome","position")]
write.table(o, file=paste0("/scratch/ji3x/full_cohort_gwas/vars/sig_",gsub(":",".",index),".txt"), sep="\t",
col.names=F, row.names=F, quote=F)
...
vars<-vars[,c("MarkerName","ID")]
vars<-vars[vars$ID!=".",]
l<-merge(l,vars,by="MarkerName", all.x=T)
rownames(l)<-l$MarkerName
l<-l[order(l$chromosome, l$position),]
l$ID<-ifelse(l$snp=="chr2.162279995.C.G","rs35337543",
ifelse(l$snp=="chr12.56042145.C.G","rs1131017",
ifelse(l$snp=="chr14.68796357.G.A","rs6573857",
ifelse(l$snp=="chr14.98011018.A.T","rs79835824",
ifelse(l$snp=="chr16.11190252.A.G","rs248832",
ifelse(l$snp=="chr16.28494339.G.C","rs151234",
ifelse(l$snp=="chr21.44280294.T.A","rs9671675",
ifelse(l$snp=="chr21.42418534.G.A","rs13048049",l$ID))))))))
...
write.snphap(dat, a1=l$Allele1, a2=l$Allele2,
             file=paste0("/scratch/ji3x/",extension,"_gwas/haplotype_5pc_diff/",gsub(":",".",index),"/init_tag.in"))

dir1<-paste0("/scratch/ji3x/",extension,"_gwas/haplotype_5pc_diff/",gsub(":",".",index),"/")
f.in=paste0(dir1,"init_tag.in")
f.out1 <- paste0(sub(".in$","",f.in),".out1")
f.out2 <- paste0(sub(".in$","",f.in),".out2")

message("running snphap (this may take a while)")
sink(file=paste0("~/programs/",extension,"/haplotype/snphap/",index,"_init_tag.sh"))
cat(paste0("/home/ji3x/software/snphap/snphap -q -nh -mi 10 -ss ",f.in," ",f.out1," ",f.out2," > ",dir1,index,"_init_tag"))
sink()


system(paste0("chmod a=rwx ~/programs/",extension,"/haplotype/snphap/",index,"_init_tag.sh"))
system(paste0("bash ~/programs/",extension,"/haplotype/snphap/",index,"_init_tag.sh"))
...
ss<-model.mi(dir=dir1,df=samples, phenotype="outcome", haps.pattern=paste0("init_tag.out2"),
...
ggsave(t,file=paste0("~/output/",extension,"_gwas/haplo_5pc_diff/",gsub(":",".",index),"_inc_tag.png"), dpi=600, height=30, width=25, units="cm")
ggsave(t,file=paste0("~/output/",extension,"_gwas/haplo_5pc_diff/",gsub(":",".",index),"_inc_tag.pdf"),height=10,width=7)
}
}
message(paste0("Done ",index))
}
lapply(orig$Marker,haplorun, base=NULL, thr=0.004)
#haplorun(base=NULL, thr=0.004,index="chr21:42416077:G:A")
#haplorun("chr10:6052734:C:T",base=NULL, thr=0.001)
