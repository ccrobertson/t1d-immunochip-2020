#post_imputation_qc_newml_res_5p_diff_dens.R

#This script performspost-imputation QC on the set of analysed SNPs:
#QC1: Remove any SNPs where the MAF is >5% from the 1000G european MAF using the G1 (UK) population only
...
l<-do.call("rbind",l)
message(paste0("Done ",cohort))
return(l)
}

combinethesnps<-function(cohort, chrom){
r<-read.table(file=paste0(outdir,cohort,"/res/chr_",chrom), header=T, as.is=T)
cols<-colnames(r)
...
r[w,"frequentist_rec_beta_1.rec.t1d.1"]<-r[w,"frequentist_rec_beta_1.rec.t1d.1"]*-1
...
r[r$all_maf<0.005,"frequentist_rec_beta_1.rec.t1d.1"]<-NA
r[r$all_maf<0.005,"frequentist_rec_wald_pvalue_1"]<-NA
r[r$all_maf<0.005,"frequentist_rec_se_1"]<-NA
...
r[w1,"frequentist_rec_beta_1.rec.t1d.1"]<-NA
r[w1,"frequentist_rec_wald_pvalue_1"]<-NA
r[w1,"frequentist_rec_se_1"]<-NA
r[w1,"all_info"]<-NA
...
r[w2,"frequentist_rec_beta_1.rec.t1d.1"]<-NA
r[w2,"frequentist_rec_wald_pvalue_1"]<-NA
r[w2,"frequentist_rec_se_1"]<-NA
r[w2,"all_info"]<-NA
...
r[w,"frequentist_rec_beta_1.rec.t1d.1"]<-NA
r[w,"frequentist_rec_wald_pvalue_1"]<-NA
r[w,"frequentist_rec_se_1"]<-NA
r[w,"all_info"]<-NA
r<-r[,cols]
return(r)
}
...
co$BETA<-co$frequentist_rec_beta_1.rec.t1d.1
co$SE<-co$frequentist_rec_se_1
co$SNP<-co$rsid
df<-co[,c("SNP","BETA","SE","NonRefAllele", "RefAllele")]
write.table(df,paste0(metdir,"/res/metal_results_",cohort,"_5pc_diff_dens.tbl"), sep=" ", col.names=T, row.names=F, quote=F)
return(df)
}
...
sink(file=paste0("~/programs/",ext,"/META/domres/","/metal_script_vcf_newml_res_5pc_diff_dens"))
cat(paste0("#metal_script_vcf_newml_res_5pc_diff_dens
...
PROCESS ",metdir,"/res/metal_results_EUR_5pc_diff_dens.tbl 

#All files are of the same format so can process immediately:
PROCESS ",metdir,"/res/metal_results_AMR_5pc_diff_dens.tbl
PROCESS ",metdir,"/res/metal_results_FIN_5pc_diff_dens.tbl
PROCESS ",metdir,"/res/metal_results_AFR_5pc_diff_dens.tbl

OUTFILE \'",metdir,"/res/METAANALYSIS_initial_5pcs_vcf_5pc_diff_dens\'
ANALYZE
...
sink(file=paste0("~/programs/",ext,"/META/domres/run_metal_vcf_newml_res_5pc_diff_dens.sh"))
cat(paste0("#!/bin/bash
#run_metal_script_vcf_newml_res_5pc_diff_dens.sh

/home/ji3x/software/generic-metal/metal metal_script_vcf_newml_res_5pc_diff_dens

#Currently can't get the results to output to where i'd like them to,
#so I am moving here:
mv METAANALYSIS1.TBL METAANALYSIS_5pcs_vcf_5pc_diff_dens.TBL
mv METAANALYSIS1.TBL.info METAANALYSIS_5pcs_vcf_5pc_diff_dens.TBL.info

mv METAANALYSIS_5pcs_vcf_5pc_diff_dens.TBL ",metdir,"/res/
mv METAANALYSIS_5pcs_vcf_5pc_diff_dens.TBL.info ",metdir,"/res/
"))
sink()
system(paste0("chmod a=rwx ~/programs/",ext,"/META/domres/run_metal_vcf_newml_res_5pc_diff_dens.sh"))
}
