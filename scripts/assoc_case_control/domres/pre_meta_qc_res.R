...
#Testing the resesive model for each SNP
...
sink(file=paste0("~/programs/",ext,"/META/snptest_scripts/res/",cohort,"_chr",chr))
...
"/samples.txt -genotype_field GP -pheno t1d -cov_all_continuous -frequentist 3 -method newml -o ",outdir,
cohort,"/res/chr_",chr))
sink()
sink(file=paste0("~/programs/",ext,"/META/snptest_scripts/res/",cohort,"_chr",chr,".sh"))
cat(paste0("#!/bin/bash
#SBATCH --time=2:00:00
...
bash ~/programs/",ext,"/META/snptest_scripts/res/",cohort,"_chr",chr,"
"))
sink()

system(paste0("sbatch ~/programs/",ext,"/META/snptest_scripts/res/",cohort,"_chr",chr,".sh"))
}
