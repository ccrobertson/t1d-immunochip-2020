#post_imputation_qc_newml_dom_5pc_diff_dens.R
...
r<-read.table(file=paste0(outdir,"/",cohort,"/dom/chr_",chrom), header=T, as.is=T)
cols<-colnames(r)
...
r[w,"frequentist_dom_beta_1.dom.t1d.1"]<-r[w,"frequentist_dom_beta_1.dom.t1d.1"]*-1
...
r[r$all_maf<0.005,"frequentist_dom_beta_1.dom.t1d.1"]<-NA
r[r$all_maf<0.005,"frequentist_dom_wald_pvalue_1"]<-NA
r[r$all_maf<0.005,"frequentist_dom_se_1"]<-NA
...
r[w1,"frequentist_dom_beta_1.dom.t1d.1"]<-NA
r[w1,"frequentist_dom_wald_pvalue_1"]<-NA
r[w1,"frequentist_dom_se_1"]<-NA
r[w1,"all_info"]<-NA
...
r[w2,"frequentist_dom_beta_1.doexit
m.t1d.1"]<-NA
r[w2,"frequentist_dom_wald_pvalue_1"]<-NA
r[w2,"frequentist_dom_se_1"]<-NA
r[w2,"all_info"]<-NA
...
r[w,"frequentist_dom_beta_1.dom.t1d.1"]<-NA
r[w,"frequentist_dom_wald_pvalue_1"]<-NA
r[w,"frequentist_dom_se_1"]<-NA
r[w,"all_info"]<-NA
r<-r[,cols]
return(r)
}
...
co$BETA<-co$frequentist_dom_beta_1.dom.t1d.1
co$SE<-co$frequentist_dom_se_1
co$SNP<-co$rsid
df<-co[,c("SNP","BETA","SE","NonRefAllele", "RefAllele")]
write.table(df,paste0(metdir,"/dom/metal_results_",cohort,"_5pc_diff_dens.tbl"), sep=" ", col.names=T, row.names=F, quote=F)
message(paste0("done ",cohort))
return(df)
}
...
sink(file=paste0("~/programs/",ext,"/META/domres/metal_script_vcf_newml_dom_5pc_diff_dens"))
cat(paste0("#metal_script_vcf_newml_5pc_diff_dens
...
PROCESS ",metdir,"/dom/metal_results_EUR_5pc_diff_dens.tbl 

#All files are of the same format so can process immediately:
PROCESS ",metdir,"/dom/metal_results_AMR_5pc_diff_dens.tbl
PROCESS ",metdir,"/dom/metal_results_FIN_5pc_diff_dens.tbl
PROCESS ",metdir,"/dom/metal_results_AFR_5pc_diff_dens.tbl

OUTFILE \'",metdir,"/METAANALYSIS_initial_5pcs_vcf_5pc_diff_dens\'
ANALYZE
...
sink(file=paste0("~/programs/",ext,"/META/domres/run_metal_vcf_newml_dom_5pc_diff_dens.sh"))
cat(paste0("#!/bin/bash
#run_metal_vcf_newml_dom_5pc_diff_dens.sh

/home/ji3x/software/generic-metal/metal metal_script_vcf_newml_dom_5pc_diff_dens
...
mv METAANALYSIS_5pcs_vcf_5pc_diff_dens.TBL ",metdir,"/dom/
mv METAANALYSIS_5pcs_vcf_5pc_diff_dens.TBL.info ",metdir,"/dom/
"))
sink()
system(paste0("chmod a=rwx ~/programs/",ext,"/META/domres/run_metal_vcf_newml_dom_5pc_diff_dens.sh"))
}
